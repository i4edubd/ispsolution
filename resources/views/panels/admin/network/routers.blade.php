        ...
        <div class="p-6">
            <div class="flex justify-between items-center">
                ...
                @can('create', \App\Models\MikrotikRouter::class)
                    <a href="{{ route('panel.admin.network.routers.create') }}" class="inline-flex items-center px-4 py-2 bg-indigo-600 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-indigo-700 focus:bg-indigo-700 active:bg-indigo-900 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 transition ease-in-out duration-150">
                            ...
                            <tr class="hover:bg-gray-50 dark:hover:bg-gray-700">
                                ...
                                <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                    <div class="flex items-center justify-end gap-2">
                                        ...
                                        <a href="{{ route('panel.admin.routers.configuration.show', ['routerId' => $router->id]) }}" class="text-blue-600 hover:text-blue-900 dark:text-blue-400 dark:hover:text-blue-300" title="Configure Router">
                                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        ...
                                        <a href="{{ route('panel.admin.mikrotik.import.index') }}?router_id={{ $router->id }}" class="text-green-600 hover:text-green-900 dark:text-green-400 dark:hover:text-green-300" title="Upload PPP Profiles">
                                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 16a4 4 0 01-.88-7.903A5 5 0 1115.9 6L16 6a5 5 0 011 9.9M15 13l-3-3m0 0l-3 3m3-3v12" />
                                            </svg>
                                        </a>
                                        <button type="button" onclick="checkRouterApi({{ $router->id }})" class="text-purple-600 hover:text-purple-900 dark:text-purple-400 dark:hover:text-purple-300" title="Check API Status">
                                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" />
                                            </svg>
                                        </button>
...
@if(session('configure_prompt') && session('router_id'))
    if (confirm('{{ session('configure_prompt') }}\n\nClick OK to configure now, or Cancel to do it later.')) {
        window.location.href = '{{ route('panel.admin.routers.configuration.show', ['routerId' => session('router_id')]) }}';
    }
@endif
