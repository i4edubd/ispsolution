...
class MikrotikService implements MikrotikServiceInterface
{
    ...
    public function getResources(int $routerId): array
    {
        try {
            ...
            if (! $router) {
                Log::error('Router not found for resource monitoring', ['router_id' => $routerId]);
                throw new \Exception('Router not found');
            }

            // Build API endpoint
            $apiUrl = "http://{$router->host}:{$router->api_port}/rest/system/resource";
            ...
            if ($response->successful()) {
                ...
                return [
                    'cpu-load' => $data['cpu-load'] ?? null,
                    'free-memory' => $data['free-memory'] ?? null,
                    'total-memory' => $data['total-memory'] ?? null,
                    'uptime' => $data['uptime'] ?? null,
                ];
            }

            Log::error('Failed to get resources from MikroTik', [
                'router_id' => $routerId,
                'response' => $response->body(),
            ]);

            throw new \Exception('Failed to get resources from router');
        } catch (\Exception $e) {
            Log::error('Error getting resources', [
                'router_id' => $routerId,
                'error' => $e->getMessage(),
            ]);
